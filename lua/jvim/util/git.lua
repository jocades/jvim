...
function M.is_repo(path)
  ...
  return JVim.file_exists(vim.fs.joinpath(path, ".git"))
end
...
function M.info(path)
  ...
  if M.is_repo(path) then
    return {
      remote = vim.fn.system("git config --get remote.origin.url"),
...
function M.diff()
  ...
  local prev = vim.split(out.stdout, "\n")
